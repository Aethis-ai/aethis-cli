...
def test_classify_bundle_id_wins_over_slug():
    # A value with both `:` and `/` is treated as a bundle ID — the API
    # never returns slugs containing colons, so this disambiguation is safe.
    assert classify_id("ns/name:20260101-deadbeef") == "bundle"
...
def test_require_bundle_id_accepts_slug():
    result = _invoke_require("aethis/uk-fsm/universal-infant")
    assert result.exit_code == 0


def test_require_bundle_id_accepts_two_segment_slug():
...
def test_require_bundle_id_rejects_unknown_with_hint():
    ...
    assert result.exit_code == 1
    assert "not a valid Bundle ID or slug" in result.output
