...
def _install_one(target: str, env: dict[str, str], *, write: bool = True) -> Path:
    ...
    if existing != desired and write:
        # Host-specific settings such as disabled tools or timeouts belong to
        # the user. Replace only this CLI's command/credential reference.
...
def _preflight_codex(env: dict[str, str]) -> None:
    """Run every refusal check that can happen before native mutation."""
    existing = _codex_get()
...
def _install_codex(env: dict[str, str]) -> str:
    existing = _codex_get()
    if existing is not None:
        ...
        if _codex_has_preservation_sensitive_settings(existing):
            raise typer.BadParameter(
                "Existing Codex Aethis MCP registration has user settings that native remove/add cannot preserve; "
                "update its selected profile manually instead."
            )
        if set(transport.get("env", {})) == _LEGACY_ENV_KEYS and not _legacy_entry_matches_selected_profile(
            transport, env["AETHIS_PROFILE"]
        ):
            raise typer.BadParameter(
                "A legacy Codex Aethis MCP registration does not match the selected saved profile. "
                "Save and select the matching profile before migrating it."
            )
    ...
    installed = _codex_get()
    if installed is None or not _codex_entry_matches(installed, env):
        raise typer.BadParameter("Codex did not confirm the expected Aethis MCP registration.")
    return "configured"
...
def install(target: str = typer.Option(..., "--target", "-t", help=f"One of: {', '.join(VALID_TARGETS)}.")) -> None:
    ...
    if "codex" in targets:
        _codex_executable()
    # Validate every target before mutating any of them. The native remove/add
    # pair remains explicitly non-atomic and reports that state if add fails.
    for current in targets:
        if current == "codex":
            _preflight_codex(env)
        else:
            _install_one(current, env, write=False)
    for current in targets:
        if current == "codex":
            success(f"codex: {_install_codex(env)} Aethis MCP for profile '{profile_name}'")
        else:
            success(f"{current}: configured Aethis MCP for profile '{profile_name}' at {_install_one(current, env)}")
